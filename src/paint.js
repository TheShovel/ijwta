'use strict';
  ...
  var sel = null;              // {type:'rect'|'ellipse'|'lasso', x,y,w,h, path:[], feather}
  var selMaskCv = null;        // canvas whose alpha is the selection (feathered)
  var selDrag = null;          // {mode:'draw'|'move', sx,sy, dx,dy, snapshot, contentCv, startSel}
  ...
  var TOOL_BUTTONS = {
    ...
    lasso: 'btnPaintToolLasso', move: 'btnPaintToolMove', transform: 'btnPaintToolTransform',
  ...
  var TOOL_OPTS = {
    ...
    lasso: 'paintToolOptsSelect', move: 'paintToolOptsMove', transform: 'paintToolOptsTransform',
  ...
  function setPaintTool(tool) {
    ...
    if (cv) {
      cv.style.cursor = (tool === 'brush' || tool === 'eraser' || tool === 'fill') ? 'crosshair' :
  ...
  function renderOverlay() {
    ...
    if (sel && selMaskCv) {
      ...
      overlayCtx.drawImage(selMaskCv, 0, 0);
  ...
  function selDown(p) {
    ...
    if (sel && selMaskCv && selPoint(p.x, p.y)) {
    ...
    if (mode === 'lasso') {
      selDrag = { mode: 'draw', type: 'lasso', sx: p.x, sy: p.y, pts: [{ x: p.x, y: p.y }] };
    } else {
      selDrag = { mode: 'draw', type: mode, sx: p.x, sy: p.y };
    }
    renderOverlay();
  }
  ...
  function selMove(p) {
    ...
    if (selDrag.mode === 'move') {
      ...
      // live preview: restore snapshot, erase sel, draw content translated
      var ctx = activeLayer.canvas.getContext('2d');
      ctx.putImageData(selDrag.snapshot, 0, 0);
      blitMovedContent(ctx, selDrag.dx, selDrag.dy);
      compositeDisplay();
      return;
    }
  ...
  function selUp() {
    ...
    if (selDrag.mode === 'move') {
      if (selDrag.dx || selDrag.dy) {
        ...
        if (sel && sel.type !== 'lasso') { sel.x += selDrag.dx; sel.y += selDrag.dy; }
    ...
    if (selDrag.type === 'lasso' && selDrag.pts.length < 3) {
      sel = null; selMaskCv = null;
    }
    selDrag = null;
    startAnts();
  ...
  // Blit the moved selection content into ctx at offset (dx,dy): erase the
  // original selection area, then draw the content clipped to the selection
  // shape translated by the same offset.
  function blitMovedContent(ctx, dx, dy) {
    if (!selDrag || !selDrag.contentCv) return;
    // erase the original selection region first
    ctx.save();
  ...
  function invertSelection() {
    ...
    selMaskCv = cv;
    // represent the inverted region as the full canvas for ants/move purposes
    sel = { type: 'rect', x: 0, y: 0, w: workW, h: workH, feather: 0, inverted: true };
    startAnts();
    compositeDisplay();
  }
  ...
  function fillDown(p) {
    ...
    var useSel = !!(byId('paintFillUseSel') && byId('paintFillUseSel').checked && selMaskCv);
  ...
  function floodFill(ctx, sx, sy, tol, contig, useSel) {
    ...
    var outA = Math.round(current.opacity * 255 * (0.4 + 0.6));
    var tolSq = tol * tol * 3;
    function match(i) {
    ...
    if (contig) {
      ...
      while (stack.length) {
        ...
        seen[pi] = 1;
        if (useSel) {
          var m = selMaskCv.getContext('2d').getImageData(x, y, 1, 1).data[3];
          if (m <= 32) continue;
        }
        var i = pi * 4;
        if (!match(i)) continue;
      ...
      for (var p = 0; p < d.length; p += 4) {
        if (match(p)) { d[p] = c.r; d[p + 1] = c.g; d[p + 2] = c.b; d[p + 3] = a0 > 0 ? Math.max(a0, outA) : outA; }
      }
      if (useSel) {
        // apply selection mask afterwards
